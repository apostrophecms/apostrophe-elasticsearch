  ...
  construct: function(self, options) {
    ...
    self.lowLevelMongoCursor = function(req, criteria, projection, options) {
      ...
      function body(callback) {
        ...
        function nextBatch() {
          if (!scrollId) {
            return module.client.search(esCriteria, gotBatch);
          } else {
            return module.client.scroll({
              scrollId: scrollId,
              scroll: '30s'
            }, gotBatch);
          }
          function gotBatch(err, response) {
            if (err) {
              if (err.status === 404) {
            ...
            return superLowLevelMongoCursor(req, {
              ...
              retrieved += response.hits.hits.length;
              if (retrieved === response.hits.total) {
                return winnow();
              }
              return nextBatch();
            });
