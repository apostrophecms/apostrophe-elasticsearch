  ...
  construct: function(self, options) {
        ...
        function nextBatch() {
          return module.client.search(esCriteria, function(err, response) {
            ...
            return superLowLevelMongoCursor(req, {
              ...
              // Avoid reaching the elasticsearch limit on
              // paging through searches. elasticsearch does offer
              // a "scroll" API (like a cursor) but its use for
              // general purpose public search is explicitly
              // not recommended.
              if (esCriteria.from + batchSize >= 10000) {
                esCriteria.size = 10000 - esCriteria.from;
                if (!esCriteria.size) {
                  return winnow();
                }
              }
    ...
    self.elasticsearchDiscoverQueries = function(req, criteria) {
      ...
      function discoverKeys(criteria) {
        // If new fields are added here they must be added to
        // mandatoryFields in index.js (_id is a special case)
