...
module.exports = {
  construct: function(self, options) {
    ...
    self.lowLevelMongoCursor = function(req, criteria, projection, options) {
          ...
          query: {
            bool: {
              must: [
                {
                  multi_match: {
                    type: elasticsearch.autocomplete ? 'phrase_prefix' : 'phrase',
                    query: elasticsearch.query,
                    // This is where boosting becomes a possibility
                    fields: [ 'title', 'tags', 'highSearchWords', 'lowSearchWords' ]
                  }
                }
              ],
      ...
      console.log(require('util').inspect(esCriteria, { depth: 10 }));
      return {
      ...
      function body(callback) {
        ...
        function nextBatch() {
          return module.client.search(esCriteria, function(err, response) {
            ...
            console.log(_.map(response.hits.hits, '_source.type'));
